...
//    ICFileParser *fp = FileParserFactory(ext);
...
double CSpectrum::maxIntensity(bool rmParentIon, double left, double right) {
    ...
    for(auto & i : m_PeakList)    {
        if(rmParentIon and m_parentMz+left<i->m_mass and i->m_mass < m_parentMz + right)        {
            continue;
...
//    cout << "What? " << cramp << endl;
    ...
    int emptySpecNum = 0;
//    cout << "Filename " << filename << " MS scan number " << cramp->getLastScan() << endl;
...
//        cout << "pointers scaninfo=" << scanInfo
//        << " p=" << p << " scannum: "
//        << scanInfo->m_data.acquisitionNum<< endl;
...
//        cout << "got scan num " << scan_num << endl;
...
//        cout << "pointer: peaks=" << peaks << " scan=" << scanInfo->m_data.acquisitionNum << endl;
        delete scanInfo;
        if (peaks) {
...
CId2RankInten::CId2RankInten(int peak_per_spec) :PeakPerSpec(peak_per_spec){
    maxScore = 0;
    for(int i = 0; i < PeakPerSpec; i ++)    {
        maxScore += (i+1)*(i+1);
    }
    cout <<  "[Info] MAX score is (42925 if peaknum=50): " << maxScore <<" and peaknum=" << PeakPerSpec<< endl;
}
