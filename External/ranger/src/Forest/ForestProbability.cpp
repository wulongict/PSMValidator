...
void ForestProbability::writePredictionFile() {
  ...
  if (predict_all) {
    ...
    for (size_t i = 0; i < predictions.size(); ++i) {
      for (size_t sample_idx = 0; sample_idx < predictions[i].size(); ++sample_idx) {//sample loop
        for (size_t class_idx = 0; class_idx < predictions[i][sample_idx].size(); ++class_idx) {// class loop
          outfile << predictions[i][sample_idx][class_idx] << " ";
        }
        outfile << std::endl;
      }
    }
